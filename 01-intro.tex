...
Let's define an equivalence relation $\~$ over $X$ as $x \~ x' \Leftrightarrow h(x) = h(x')$. 
...
First such improvement is the \emph{DPLL algorithm} (Davis-Putnam-Logemann-Loveland) which improves upon the naive backtracking algorithm by applying two rules at each step:
...
With \emph{conflict-driven clause learning} (CDCL) we can avoid this by the use of \emph{implication graph} and \emph{learnt clauses}.
...
\begin{align*}
(x_1 = v_1 \land \dots \land x_k = v_k) &\Rightarrow \text{conflict} \\
\overline{\text{conflict}} &\Rightarrow \overline{(x_1 = v_1 \land \dots \land x_k = v_k)} \\
\top &\Rightarrow (x_1 = \overline{v_1} \lor \dots \lor x_k = \overline{v_k}) \\
L &:= (x_1^{(\overline{v_1})} \lor \dots \lor x_k^{(\overline{v_k})})
\end{align*}

The notation $x^{(v)}$ is used to represent $x$ if $v=1$ and $\overline{x}$ if $v=0$.
